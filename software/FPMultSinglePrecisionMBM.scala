...
class FPMultSinglePrecisionMBM(val width: Int = 32, val k: Int = 24, val expsz: Int = 8, val mntsz: Int = 23) extends Module {
  // Parameters
  ...
  val io = IO(new Bundle {
    ...
    */
    ...
    o_exp1 := (a_exp.asSInt +& b_exp.asSInt - bias.S)
    ...
    */
    ...
    val APPLY_ROUNDING = true.B  // Set this to a parameter or flag as required
...
*/
    ...
    val o_mnt = Reg(UInt(p.W))              // equivalent to reg [p-1:0] o_mnt;
    //val o_exp4 = Reg(UInt(expsz.W))          // equivalent to reg [expsz-1:0] o_exp4
    val o_exp4 = Reg(SInt(expsz.W))
...
class SteeringLogicREM16rd(val sz: Int = 24, val lgsz: Int = 5, val k: Int = 24) extends Module {
  ...
  // Registers
  val mantX = Reg(UInt(k.W))
  val mantY = Reg(UInt(k.W))
